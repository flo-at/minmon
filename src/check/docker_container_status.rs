...
impl DockerContainerStatus {
    ...
    async fn container_running_and_healthy(
        ...
        let health = state
            .health
            .ok_or_else(|| Error(String::from("Could not read container health.")))?;
        let health_status = health
            .status
            .ok_or_else(|| Error(String::from("Could not read container health status.")))
            .map(|x| {
                x == bollard::models::HealthStatusEnum::HEALTHY
                    || x == bollard::models::HealthStatusEnum::NONE
            })?;
        Ok(status && health_status)
    }
}
...
impl DataSource for DockerContainerStatus {
    ...
    async fn get_data(&self) -> Result<Vec<Result<Self::Item>>> {
        let docker =
            bollard::Docker::connect_with_unix(&self.socket_path, 0, bollard::API_DEFAULT_VERSION)
